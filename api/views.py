...
from .serializers import ClientRegistrationSerializer,UserLoginSerializer,PharmacieRegistrationSerializer
...
class ClientRegistrationAPIView(APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid(raise_exception=True):
            ...
            if new_user:
                # Créez et enregistrez l'access token
                access_token = generate_access_token(new_user)
                AccessToken.objects.create(user=new_user, token=access_token)

                # Créez et enregistrez le refresh token
                refresh_token = generate_refresh_token(new_user)
                RefreshToken.objects.create(user=new_user, token=refresh_token)
...
class ClientViewAPI(APIView):
	...
	def get(self, request):
		...
		user_serializer = ClientRegistrationSerializer(user)
